...
int maxFlow(int source, int destination, int number_of_nodes)
{
...
int main()
{
		ios_base::sync_with_stdio(false);
	 	cin.tie(NULL);
	 	int a, b, c, src,n, m , dest, number_of_nodes, edges;
	 	cin >> n >> m >> edges ;
	 	FOR(i,0,edges)
	 	{
	 		cin >> a >> b;
	 	...
	 	FOR(i,1,n+1)addEdge(0,i,1);
	 	FOR(i,1,m+1)addEdge(n+i,n + m + 1 ,1);
	 	src = 0, dest = n + m + 1;
	 	cout<<maxFlow(src, dest,n+m+2);
		return 0;
}