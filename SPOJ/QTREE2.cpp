...
void queryseg(int s,int e,int index,int l,int r) 
{
	...
	queryarr[index]=(queryarr[index<<1]+queryarr[(index<<1)+1]);
}
...
int findancestor(int vertex,int dist)
{
	int index=0;
	while(dist>0)
	{
...
void kquery(int u,int v,int k)
{
	k--;
	int lca=LCA(u,v),ans;
	if(depth[u]-depth[lca]>=k)
		ans=findancestor(u,k);
	else
		ans=findancestor(v,(depth[v]-depth[lca])-(k-(depth[u]-depth[lca])));
	cout<<ans+1<<ln;
}
int main() 
{
